...
class Profile:
    ...
    # ROB-1275: upstream serving-model observation for unversioned ClinePass slugs.
    # ``served_slug`` is the request slug sent to the vendor; ``upstream_model`` is
    # the model actually serving it as reported by the API echo (`model` field),
    # and ``upstream_as_of`` is the ISO date that observation was taken. Groups are
    # re-anchored silently, so the recorded value is the authoritative drift baseline.
...
GRADE_TABLE: dict[Grade, list[Profile]] = {
    ...
    "A+": [
        ...
        Profile(
            ...
            **_aa_agent_benchmark(57.0, "qwen3.8-max", "default", harness="claude-code"),
            ...
            served_slug="cline-pass/qwen3.8-max",
    ...
    "B": [
        ...
        Profile(
            ...
            **_aa_agent_benchmark(43.0, "glm-5.2", "default", harness="claude-code"),
            ...
            aa_model_id="glm-5-2",
            served_slug="cline-pass/glm-5.2",
            upstream_model="private/glm-5p2-wave",
            upstream_as_of="2026-08-15",
        ),
    ...
    "C": [
        ...
        Profile(
            ...
            served_slug="cline-pass/qwen3.7-max",
        ),
        Profile(
            "oc-minimax-m3",
            "minimax-m3",
            ...
            served_slug="cline-pass/minimax-m3",
        ),
        Profile(
            "oc-omni",
            "OmniRoute free",
...
def _pool_cutoff(provider_id: str, pool_class: PoolClass) -> tuple[float, str | None]:
    """task #638 — ``[pools.<p>] cutoff`` 오버라이드. 무효값은 기본값 폴백 + status."""
    return get_cutoff(provider_id, _usage_cutoff(pool_class))


def _exhaust_display(over: _WindowState, cutoff: float) -> str:
    """task #638 범위3 — 남은 양이 오해 없이 보이는 'N% 사용 · M% 남음 · 차단선 K%' 문구."""
    return f"{over.used_pct:g}% 사용 · {over.remaining_pct:g}% 남음 · 차단선 {cutoff:g}% — 소진"


def _exhaust_suffix(cutoff_status: str | None, notify_status: str | None) -> str:
    parts = [part for part in (cutoff_status, notify_status) if part]
...
def _observe_exhaustion(
    ...
    return exhaust.observe(
        provider_id,
        exhausted=over is not None,
        used_pct=over.used_pct if over is not None else None,
        cutoff=cutoff,
